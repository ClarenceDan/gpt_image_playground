...
export default function DetailModal() {
  ...
  const handleSaveCurrentOutput = async () => {
    if (!currentOutputImageSrc) return
    try {
      await downloadImage(currentOutputImageSrc)
          ...
          {task.status === 'done' && outputLen > 0 && currentOutputImageSrc && (
            <>
              ...
              <button
                ...
                onClick={(e) => {
                  e.stopPropagation()
                  void handleSaveCurrentOutput()
                }}
                className="absolute right-4 top-[15px] flex h-8 w-8 items-center justify-center rounded-full bg-black/50 text-white shadow-sm backdrop-blur-sm transition hover:bg-black/65"
                aria-label="保存图片"
                title="保存图片"
              >
